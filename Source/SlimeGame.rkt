                     ...
                     (lambda (canvas dc)
                       ...
                       (define (iter dc i score1 score2)
                         (if (= 7 i)
                             0
                             (begin
                               (if (< i score1) (send dc draw-ellipse (+ 24 (* 60 i)) 24 28 28) #f)
                               (if (< i score2) (send dc draw-ellipse (- (- windowXbound 51)  (* 60 i)) 24 28 28) #f)
  ...
  (if (equal? (cdr (ball 'get_pos)) (- windowYbound 18))
      (begin 
        (if (< ( + (ball 'get_rad) (car (ball 'get_pos))) (/ windowXbound 2) )
            (if (= player2score 6)
                 (begin (set! player2score (+ 1 player2score)) (send gamecanvas on-paint) (set! player2score 0) (set! player1score 0) (set! lastToscore #f)
                       ...
                       (send (send gamecanvas get-dc) draw-text "Player 2 Wins!" 300 100)
                       (send (send gamecanvas get-dc) set-font (make-object font% 20 'default))
                       (send (send gamecanvas get-dc) draw-text "Press Space to Restart" 350 160))
                
                (begin (set! player2score (+ 1 player2score)) (send gamecanvas on-paint) (set! lastToscore #t)
                       (send (send gamecanvas get-dc) set-font (make-object font% 40 'default))
                       (send (send gamecanvas get-dc) set-text-foreground (make-object color% 255 255 255 .99))
            ...
            (if (= player1score 6)
                (begin (set! player1score (+ 1 player1score)) (send gamecanvas on-paint) (set! player2score 0) (set! player1score 0) (set! lastToscore #f)
                       (send (send gamecanvas get-dc) set-font (make-object font% 40 'default))
                       (send (send gamecanvas get-dc) set-text-foreground (make-object color% 255 255 255 .99))
                       (send (send gamecanvas get-dc) draw-text "Player 1 Wins!" 300 100)
                       (send (send gamecanvas get-dc) set-font (make-object font% 20 'default))
                       (send (send gamecanvas get-dc) draw-text "Press Space to Restart" 350 160))
                (begin (set! player1score (+ 1 player1score)) (send gamecanvas on-paint) (set! lastToscore #f)
                       (send (send gamecanvas get-dc) set-font (make-object font% 40 'default))
                       (send (send gamecanvas get-dc) set-text-foreground (make-object color% 255 255 255 .99))
