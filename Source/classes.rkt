  ...
  (define (dispatch op)
    (cond ((eq? op 'get_pos) (cons pos_x pos_y))
          ...
          ((eq? op 'move) (begin
                            ...
                            (if (<= pos_y bottom_bound) (set! vel_y (+ vel_y 0.4)) (begin (set! vel_y 0) (set! pos_y bottom_bound)
                            ))) ;gravity

          ((eq? op 'collision) (lambda (Slime)  (if (<= (distance Slime ball) (+ radius (Slime 'get_rad)))
                                                     (begin (set! vel_y (* -15 (/ (- (+ (cdr (Slime 'get_pos)) (Slime 'get_rad)) (+ (cdr (ball 'get_pos)) (ball 'get_rad))) (distance Slime ball))))
                                                            (set! vel_x (* -15 (/ (- (+ (car (Slime 'get_pos)) (Slime 'get_rad)) (+ (car (ball 'get_pos)) (ball 'get_rad))) (distance Slime ball)))))
                                                     #f)))
          ((eq? op 'wall) (if (< (+ pos_x vel_x) left_bound)
                              (begin
                                (set! vel_x (- vel_x))
                                (set! pos_x left_bound))
                                (if (> (+ pos_x vel_x) right_bound)
                                    (begin
                                      (set! vel_x (- vel_x))
                                      (set! pos_x right_bound))
                                    (values))))
