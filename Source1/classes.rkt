...
(define (make_object pos_x pos_y vel_x vel_y radius left_bound right_bound bottom_bound)
  (define (dispatch op)
    (cond ((eq? op 'get_pos) (cons pos_x pos_y))
          ...
          ((eq? op 'collision) (let ((x (car (Slime1 'get_pos)))
                                                     (y (+ (cdr (Slime1 'get_pos)) (Slime1 'get_rad)))
                                                     (r (Slime1 'get_rad)))
                                                 (if (and (>= (+ pos_x (* 2 radius)) x) (<= pos_x (+ x (* 2 r))))
                                                     (if (>= (+ pos_y (* 2 radius)) (- y (- r (abs (- (+ x r) pos_x)))))
                                                         (let ((x (- (+ pos_x radius) (+ x r)))
                                                               (y (- r x)))
                                                           (set! vel_x (floor (/ x 5)))
                                                           (set! vel_y (/ y 5)))
                                                         #f)
                                                     #f))
                               (let ((x (car (Slime2 'get_pos)))
                                                     (y (+ (cdr (Slime2 'get_pos)) (Slime2 'get_rad)))
                                                     (r (Slime2 'get_rad)))
                                                 (if (and (<= (+ pos_x (* 2 radius)) x) (>= pos_x (+ x (* 2 r))))
                                                     (if (>= (+ pos_y (* 2 radius)) (- y (- r (abs (- (+ x r) pos_x)))))
                                                         (let ((x (- (+ pos_x radius) (+ x r)))
